...
async def list_skills() -> list[SkillOut]:
    return [
        SkillOut(name=s.name, description=s.description, source=s.source, files=s.files) for s in all_skills()
    ]
...
async def list_designs(session: AsyncSession = Depends(get_session)) -> list[DesignSummaryOut]:
    rows = (await session.execute(select(Design).order_by(Design.updated_at.desc()))).scalars().all()
    return [
        DesignSummaryOut(
            id=d.id,
            title=d.title,
...
async def create_design(body: DesignCreate, session: AsyncSession = Depends(get_session)) -> DesignOut:
    ...
    design = Design(title=title, brief=body.brief, chat_id=chat.id, model_id=model.id, working_dir=folder)
    session.add(design)
    await session.flush()
