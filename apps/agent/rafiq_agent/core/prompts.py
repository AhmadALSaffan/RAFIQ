...
MEMORY_NOTE = (
    "وعندك أداة memory_save لتحفظ شي للمحادثات الجاية. استخدمها بس لما المستخدم يطلب صراحةً إنك تتذكّر "
    "(«تذكّر إني…»)، أو لما يذكر تفضيل واضح وثابت. جملة وحدة بكل مرة، وما تحفظ معلومات عابرة أو حسّاسة."
)

PLAN_PROMPT = (
    "قبل ما تنفّذ أي شي: اكتب خطة مرقّمة مختصرة لهالمهمة (الخطوات بالترتيب، الملفات اللي رح تلمسها، "
    "والأوامر اللي رح تشغّلها). لا تستدعي أي أداة ولا تعدّل شي هلأ. الخطة بس، وبعدها توقّف."
)

PLAN_APPROVED = "المستخدم وافق على الخطة. نفّذها هلأ خطوة خطوة، وبلّغ عن أي انحراف عنها."

STEP_NOTE = "هالمهمة بوضع «خطوة خطوة»: كل كتابة أو أمر بيستنى موافقة المستخدم. اشرح كل خطوة بسطر قبلها."
