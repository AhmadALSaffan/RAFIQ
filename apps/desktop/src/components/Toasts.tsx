      ...
      <AnimatePresence initial={false}>
        {toasts.map((t) => (
          <motion.div
            ...
            className={`pointer-events-auto flex items-start gap-3 rounded-xl border px-4 py-3 shadow-lg ${t.tone === "approval" ? "pending-ring" : ""}`}
            style={{ borderColor: toneColor[t.tone], background: "var(--color-surface)" }}
            role="status"
          >
            <span className="mt-0.5 shrink-0" style={{ color: toneColor[t.tone] }}>
              {t.tone === "approval" ? <ShieldIcon className="h-5 w-5" /> : t.tone === "success" ? <DrawnCheck className="h-5 w-5" /> : <AlertIcon className="h-5 w-5" />}
            </span>
            <div className="min-w-0 flex-1">
              ...
              {t.onAction && (
                <button
                  ...
                  style={{ background: toneColor[t.tone], color: t.tone === "approval" ? "var(--color-accent-ink)" : "white" }}
                >
                  {t.actionLabel ?? "افتح"}
                </button>
              )}
            </div>
            <button onClick={() => onDismiss(t.id)} aria-label="إغلاق" className="shrink-0 rounded p-0.5" style={{ color: "var(--color-ink-muted)" }}>
